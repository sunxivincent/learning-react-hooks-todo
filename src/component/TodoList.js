      ...
      <ul className="text-white p-0">
        {state.todos.map(todo => (
          <li key={todo.id} className="flex items-center bg-orange-dark border-black border-dashed border-2 my-2 py-4">
            ...
            <button><img src="http://icon.now.sh/edit/0050c5" className="h-6"/></button>
            <button onClick={() => dispatch({ type: "REMOVE_TODO", payload: todo })}><img src="http://icon.now.sh/delete/8b0000" className="h-6"/></button>
          </li>
        ))}
      </ul>
    </div>
